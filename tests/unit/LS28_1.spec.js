...
describe("LS28_1 tests", () => {
  test("adds parentheses to A", () => {
    const input = "A";
    expect(convert(input, "LS28_1")).toBe("(A)");
  });

  test("adds parentheses to conjunction", () => {
    const input = "A∧B";
    expect(convert(input, "LS28_1")).toBe("(A∧B)");
  });
  ...
  test("adds parentheses to formula starting with ∀", () => {
    const input = "∀xF(x)";
    expect(convert(input, "LS28_1")).toBe("(∀xF(x))");
  });
  ...
  test("adds parentheses to complex expression", () => {
    ...
    expect(convert(input, "LS28_1")).toBe("((∀x∃yA(x)∧B(y))⇔C(z)∧D(f(x),g(z)))");
  });

  test("throws error on extra parentheses in the end", () => {
    const input = "A)";
    expect(() => convert(input, "LS28_1")).toThrow();
  });
