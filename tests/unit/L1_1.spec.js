...
describe("L1_1 tests", () => {
  test("simplifies simple predicate", () => {
    const input = "A(x)∧A(x)";
    expect(convert(input, "L1_1")).toBe("A(x)");
  });
  ...
  test("simplifies conjunction in parentheses", () => {
    const input = "(A∧B)∧(A∧B)";
    expect(convert(input, "L1_1")).toBe("(A∧B)");
  });
  ...
  test("simplifies predicate with ∀", () => {
    ...
    expect(convert(input, "L1_1")).toBe("∀xF(x)");
  });
  ...
  test("simplifies complex expression", () => {
    ...
    expect(convert(input, "L1_1")).toBe("∀x∃y∀z(F(x)∧G(y,z)⇒A(t,u,v,f(w)))");
  });

  test("returns null when left and right side don't match", () => {
    const input = "(A∨B)∧(A∨C)";
    expect(convert(input, "L1_1")).toBe(null);
  });
