...
describe("LS28_1 tests", () => {
  test("removes parentheses from predicate", () => {
    const input = "(A(x,y,z))";
    expect(convert(input, "LS28_2")).toBe("A(x,y,z)");
  });

  test("removes parentheses from negation", () => {
    const input = "(¬A)";
    expect(convert(input, "LS28_2")).toBe("¬A");
  });

  test("removes parentheses from conjunction", () => {
    const input = "(A∧B)";
    expect(convert(input, "LS28_2")).toBe("A∧B");
  });
  ...
  test("removes parentheses from formula starting with ∀", () => {
    const input = "(∀xF(x))";
    expect(convert(input, "LS28_2")).toBe("∀xF(x)");
  });
  ...
  test("removes parentheses from complex expression", () => {
    ...
    expect(convert(input, "LS28_2")).toBe("∃x∀y(F(x)⇔G(y))∧∀zH(x,y,z)");
  });

  test("returns null on expression that does not have parentheses surrounding it", () => {
    const input = "(A∧¬B)∨C";
    expect(convert(input, "LS28_2")).toBe(null);
  });
});
