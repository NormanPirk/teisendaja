...
describe("Tests regarding formula object", () => {
  ...
  test("returns correct TeX", () => {
    ...
    expect(formula.toTex()).toBe(
      "\\underline{A\\land B}\\Rightarrow C^{\\;\\;\\;L3}"
    );
  });
  ...
  test("returns correct PDF", () => {
    ...
    expect(formula.getPDFContent(false)).toStrictEqual({
      text: [
        { text: "A∧B⇒C" },
  ...
  test("returns correct PDF", () => {
    ...
    expect(formula.getPDFContent(true)).toStrictEqual({
      text: [
        { text: "≡\t" },
